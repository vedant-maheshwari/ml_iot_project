...
class Application:
    ...
    def __init__(self):
        ...
        self.video_loop()
        self.csv_btn = tk.Button(self.root, text="Save to CSV", font=("Courier", 20), command=self.save_to_csv)
        self.csv_btn.place(x=1105, y=680)  # Adjust position as needed

        self.send_btn = tk.Button(self.root, text="Send CSV", font=("Courier", 20), command=self.send_csv)
        self.send_btn.place(x=1105, y=730)  # Adjust position
    ...
    def save_to_csv(self):
        with open("sign_language_output.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([self.str])
        print("Text saved to sign_language_output.csv")
    ...
    def send_csv(self):
        broker_address = "test.mosquitto.org"  # Replace with your MQTT broker IP if using a private broker
        topic = "sign_language/speech"

        # Read CSV file content
        with open("sign_language_output.csv", "r") as file:
            csv_data = file.read()

        client = mqtt.Client()
        client.connect(broker_address, 1883, 60)  # Connect to the MQTT broker
        client.publish(topic, csv_data)  # Publish CSV content
        client.disconnect()
        
        print("CSV file sent via MQTT!")
